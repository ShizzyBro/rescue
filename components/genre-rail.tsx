...
export const GenreRail = memo(function GenreRail({ selectedGenre, onGenreChange }: GenreRailProps) {
  ...
  return (
    <div
      className="relative py-3 sticky z-30 top-0"
      style={{
      ...
      <div className="mx-auto max-w-[1320px] px-4 sm:px-6 lg:px-8">
        <div className="relative">
          {showLeftArrow && (
            <button
              onClick={() => scroll("left")}
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-7 h-7 flex items-center justify-center glass rounded-full shadow-lg"
          ...
          {showRightArrow && (
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-7 h-7 flex items-center justify-center glass rounded-full shadow-lg"
            ...
            {genres.map((genre) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-semibold transition-all duration-200 whitespace-nowrap",
                  ...
                  style={
                    isActive
                      ? {
                          ...
                          color: cfg.color,
                          boxShadow: `0 0 14px ${cfg.color}25, inset 0 1px 0 oklch(1 0 0 / 0.08)`,
                        }
                      : {
                ...
                >
                  <Icon className="h-3 w-3 shrink-0" style={{ color: isActive ? cfg.color : "oklch(0.62 0.02 250)" }} />
                  {genre}
                </button>
              )
            })}
